...
# Mount the appropriate andor etc directory
ANDOR_VER=2.102.30034.0
if [ ! -e /usr/local/etc/andor/version -o "`cat /usr/local/etc/andor/version`" != "${ANDOR_VER}" ]
then
    mount --bind /reg/g/pcds/package/external/andor-${ANDOR_VER}/etc /usr/local/etc/andor
fi
